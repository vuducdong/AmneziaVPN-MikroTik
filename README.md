...
В репозитории присутствуют шаблоны для самостоятельной сборки Docker контейнеров в каталоге **"Containers"**. Предварительно шаблон сборки нужно собрать через Docker или скачать готовые контейнеры из [Docker Hub](https://hub.docker.com/u/catesin). В конфигурации шаблонов предусмотрена маршрутизация IPv6. Контейнеры делятся на три архитектуры **ARM, ARM64 и x86**. 
